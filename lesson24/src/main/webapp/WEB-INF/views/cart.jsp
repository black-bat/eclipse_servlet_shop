...
<br><br>
  ...
  <table id="cart_table" border="1">
  	...
  	<c:forEach var="item" items="${cart}" varStatus="num">
  	<tr>
 	 		<td>${num.count}</td>
  			<td>${item.title}</td>
  			<td>${item.price}</td>
  			...
  			<td><button type="submit" onclick="dellProduct(${item.product_id})">Del</button></td>
  	</tr>
  	</c:forEach>
  </table>
  </div>
  <div><button id="orderButton" onclick="order()" type="button" method="POST" class="btn btn-primary">Order</button></div>
